import type { FC } from "react";

import { CentralIconBase, type CentralIconProps } from "./CentralIconBase";

export const IconAsterisk: FC<CentralIconProps> = ({
	variant = "outlined",
	...props
}) => (
	<CentralIconBase {...props} ariaLabel="asterisk, placeholder">
		{variant === "filled" ? (
			<>
				<path
					clipRule="evenodd"
					d="M12.003 2C12.4173 2 12.753 2.33579 12.753 2.75V10.7015L19.6398 6.72548C19.9985 6.51837 20.4572 6.64128 20.6643 7C20.8714 7.35872 20.7485 7.81741 20.3898 8.02452L13.5035 12.0003L20.3888 15.9755C20.7475 16.1826 20.8704 16.6413 20.6633 17C20.4562 17.3587 19.9975 17.4816 19.6388 17.2745L12.753 13.299V21.25C12.753 21.6642 12.4173 22 12.003 22C11.5888 22 11.253 21.6642 11.253 21.25V13.2996L4.36828 17.2745C4.00956 17.4816 3.55087 17.3587 3.34376 17C3.13666 16.6413 3.25956 16.1826 3.61828 15.9755L10.5035 12.0003L3.61731 8.02452C3.25859 7.81741 3.13569 7.35872 3.34279 7C3.5499 6.64128 4.00859 6.51837 4.36731 6.72548L11.253 10.701V2.75C11.253 2.33579 11.5888 2 12.003 2Z"
					fill="currentColor"
					fillRule="evenodd"
				/>
			</>
		) : (
			<>
				<path
					d="M12.0029 2.75V21.25M3.99219 7.375L20.0137 16.625M3.99316 16.625L20.0146 7.375"
					stroke="currentColor"
					strokeLinecap="round"
					strokeWidth="1.5"
				/>
			</>
		)}
	</CentralIconBase>
);

export default IconAsterisk;
