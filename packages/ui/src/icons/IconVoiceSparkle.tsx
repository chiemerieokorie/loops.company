import type { FC } from "react";

import { CentralIconBase, type CentralIconProps } from "./CentralIconBase";

export const IconVoiceSparkle: FC<CentralIconProps> = ({
	variant = "outlined",
	...props
}) => (
	<CentralIconBase {...props} ariaLabel="voice-sparkle, ai, sound">
		{variant === "filled" ? (
			<>
				<path
					d="M7.75 3C8.16421 3 8.5 3.33579 8.5 3.75V20.25C8.5 20.6642 8.16421 21 7.75 21C7.33579 21 7 20.6642 7 20.25V3.75C7 3.33579 7.33579 3 7.75 3Z"
					fill="currentColor"
				/>
				<path
					d="M16 13C16.2409 13 16.4574 13.1482 16.5439 13.373L17.2402 15.1855C17.3418 15.4494 17.5506 15.6582 17.8145 15.7598L19.627 16.4561C19.8518 16.5426 20 16.7591 20 17C20 17.2409 19.8518 17.4574 19.627 17.5439L17.8145 18.2402C17.5506 18.3418 17.3418 18.5506 17.2402 18.8145L16.5439 20.627C16.4574 20.8518 16.2409 21 16 21C15.7591 21 15.5426 20.8518 15.4561 20.627L14.7598 18.8145C14.6582 18.5506 14.4494 18.3418 14.1855 18.2402L12.373 17.5439C12.1482 17.4574 12 17.2409 12 17C12 16.7591 12.1482 16.5426 12.373 16.4561L14.1855 15.7598C14.4494 15.6582 14.6582 15.4494 14.7598 15.1855L15.4561 13.373C15.5426 13.1482 15.7591 13 16 13Z"
					fill="currentColor"
				/>
				<path
					d="M3.75 9C4.16421 9 4.5 9.33579 4.5 9.75V14.25C4.5 14.6642 4.16421 15 3.75 15C3.33579 15 3 14.6642 3 14.25V9.75C3 9.33579 3.33579 9 3.75 9Z"
					fill="currentColor"
				/>
				<path
					d="M12 7C12.4142 7 12.75 7.33579 12.75 7.75V13.75C12.75 14.1642 12.4142 14.5 12 14.5C11.5858 14.5 11.25 14.1642 11.25 13.75V7.75C11.25 7.33579 11.5858 7 12 7Z"
					fill="currentColor"
				/>
				<path
					d="M20.25 9C20.6642 9 21 9.33579 21 9.75V13.75C21 14.1642 20.6642 14.5 20.25 14.5C19.8358 14.5 19.5 14.1642 19.5 13.75V9.75C19.5 9.33579 19.8358 9 20.25 9Z"
					fill="currentColor"
				/>
				<path
					d="M16.25 5C16.6642 5 17 5.33579 17 5.75V10.75C17 11.1642 16.6642 11.5 16.25 11.5C15.8358 11.5 15.5 11.1642 15.5 10.75V5.75C15.5 5.33579 15.8358 5 16.25 5Z"
					fill="currentColor"
				/>
			</>
		) : (
			<>
				<path
					d="M7.75 3.75V20.25"
					stroke="currentColor"
					strokeLinecap="round"
					strokeWidth="1.5"
				/>
				<path
					d="M3.75 9.75V14.25"
					stroke="currentColor"
					strokeLinecap="round"
					strokeWidth="1.5"
				/>
				<path
					d="M12 7.75V13.75"
					stroke="currentColor"
					strokeLinecap="round"
					strokeWidth="1.5"
				/>
				<path
					d="M16.25 5.75V10.75"
					stroke="currentColor"
					strokeLinecap="round"
					strokeWidth="1.5"
				/>
				<path
					d="M20.25 9.75V13.75"
					stroke="currentColor"
					strokeLinecap="round"
					strokeWidth="1.5"
				/>
				<path
					d="M17.2405 15.1852L16.5436 13.3733C16.4571 13.1484 16.241 13 16 13C15.759 13 15.5429 13.1484 15.4564 13.3733L14.7595 15.1852C14.658 15.4493 14.4493 15.658 14.1852 15.7595L12.3733 16.4564C12.1484 16.5429 12 16.759 12 17C12 17.241 12.1484 17.4571 12.3733 17.5436L14.1852 18.2405C14.4493 18.342 14.658 18.5507 14.7595 18.8148L15.4564 20.6267C15.5429 20.8516 15.759 21 16 21C16.241 21 16.4571 20.8516 16.5436 20.6267L17.2405 18.8148C17.342 18.5507 17.5507 18.342 17.8148 18.2405L19.6267 17.5436C19.8516 17.4571 20 17.241 20 17C20 16.759 19.8516 16.5429 19.6267 16.4564L17.8148 15.7595C17.5507 15.658 17.342 15.4493 17.2405 15.1852Z"
					fill="currentColor"
				/>
			</>
		)}
	</CentralIconBase>
);

export default IconVoiceSparkle;
