import type { FC } from "react";

import { CentralIconBase, type CentralIconProps } from "./CentralIconBase";

export const IconChipSimple: FC<CentralIconProps> = ({
	variant = "outlined",
	...props
}) => (
	<CentralIconBase {...props} ariaLabel="chip-simple, processor">
		{variant === "filled" ? (
			<>
				<path d="M14.5 14.5H9.5V9.5H14.5V14.5Z" fill="currentColor" />
				<path
					clipRule="evenodd"
					d="M14.25 1C14.6642 1 15 1.33579 15 1.75V3H17.25C19.3211 3 21 4.67893 21 6.75V9H22.25L22.3271 9.00391C22.7051 9.04253 23 9.36183 23 9.75C23 10.1382 22.7051 10.4575 22.3271 10.4961L22.25 10.5H21V13.5H22.25L22.3271 13.5039C22.7051 13.5425 23 13.8618 23 14.25C23 14.6382 22.7051 14.9575 22.3271 14.9961L22.25 15H21V17.25C21 19.3211 19.3211 21 17.25 21H15V22.25C15 22.6642 14.6642 23 14.25 23C13.8358 23 13.5 22.6642 13.5 22.25V21H10.5V22.25C10.5 22.6642 10.1642 23 9.75 23C9.33579 23 9 22.6642 9 22.25V21H6.75C4.67893 21 3 19.3211 3 17.25V15H1.75C1.33579 15 1 14.6642 1 14.25C1 13.8358 1.33579 13.5 1.75 13.5H3V10.5H1.75C1.33579 10.5 1 10.1642 1 9.75C1 9.33579 1.33579 9 1.75 9H3V6.75C3 4.67893 4.67893 3 6.75 3H9V1.75C9 1.33579 9.33579 1 9.75 1C10.1642 1 10.5 1.33579 10.5 1.75V3H13.5V1.75C13.5 1.33579 13.8358 1 14.25 1ZM8.75 8C8.36183 8 8.04253 8.29488 8.00391 8.67285L8 8.75V15.25C8 15.6642 8.33579 16 8.75 16H15.25C15.6642 16 16 15.6642 16 15.25V8.75C16 8.36183 15.7051 8.04253 15.3271 8.00391L15.25 8H8.75Z"
					fill="currentColor"
					fillRule="evenodd"
				/>
			</>
		) : (
			<>
				<path
					d="M20.25 3.75H3.75V20.25H20.25V3.75Z"
					stroke="currentColor"
					strokeLinecap="round"
					strokeLinejoin="round"
					strokeWidth="1.5"
				/>
				<path
					d="M15.25 8.75H8.75V15.25H15.25V8.75Z"
					stroke="currentColor"
					strokeLinecap="round"
					strokeLinejoin="round"
					strokeWidth="1.5"
				/>
				<path
					d="M9.75 3.75V1.75"
					stroke="currentColor"
					strokeLinecap="round"
					strokeLinejoin="round"
					strokeWidth="1.5"
				/>
				<path
					d="M14.25 3.75V1.75"
					stroke="currentColor"
					strokeLinecap="round"
					strokeLinejoin="round"
					strokeWidth="1.5"
				/>
				<path
					d="M9.75 22.25V20.25"
					stroke="currentColor"
					strokeLinecap="round"
					strokeLinejoin="round"
					strokeWidth="1.5"
				/>
				<path
					d="M14.25 22.25V20.25"
					stroke="currentColor"
					strokeLinecap="round"
					strokeLinejoin="round"
					strokeWidth="1.5"
				/>
				<path
					d="M3.75 9.75H1.75"
					stroke="currentColor"
					strokeLinecap="round"
					strokeLinejoin="round"
					strokeWidth="1.5"
				/>
				<path
					d="M3.75 14.25H1.75"
					stroke="currentColor"
					strokeLinecap="round"
					strokeLinejoin="round"
					strokeWidth="1.5"
				/>
				<path
					d="M22.25 9.75H20.25"
					stroke="currentColor"
					strokeLinecap="round"
					strokeLinejoin="round"
					strokeWidth="1.5"
				/>
				<path
					d="M22.25 14.25H20.25"
					stroke="currentColor"
					strokeLinecap="round"
					strokeLinejoin="round"
					strokeWidth="1.5"
				/>
			</>
		)}
	</CentralIconBase>
);

export default IconChipSimple;
