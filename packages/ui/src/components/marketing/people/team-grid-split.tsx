import { defaultSplitMembers } from "./data";
import type { TeamMember } from "./types";

export interface TeamGridSplitProps {
	description?: string;
	members?: TeamMember[];
	title?: string;
}

export function TeamGridSplit({
	title = "Our incredible leadership team",
	description = "Our team thrives on collaboration, creativity, and a shared commitment to excellence.",
	members = defaultSplitMembers,
}: TeamGridSplitProps) {
	return (
		<section className="bg-background py-16 md:py-32">
			<div className="@container mx-auto max-w-5xl px-6">
				<div className="grid gap-16 lg:grid-cols-5 lg:gap-24">
					<div className="lg:col-span-2">
						<h2 className="max-w-xs text-balance font-semibold text-4xl text-foreground">
							{title}
						</h2>
						<p className="mt-4 text-lg text-muted-foreground">{description}</p>
					</div>
					<div className="grid grid-cols-2 gap-16 sm:grid-cols-3 md:max-lg:grid-cols-4 lg:col-span-3">
						{members.map((member) => (
							<div key={member.name}>
								<div className="relative aspect-5/6 w-24 overflow-hidden rounded-2xl shadow-black/3 shadow-md before:absolute before:inset-0 before:z-1 before:rounded-2xl before:border before:border-foreground/6.5">
									<img
										alt={member.name}
										className="size-full object-cover grayscale"
										decoding="async"
										height="540"
										loading="lazy"
										src={member.image}
										style={{ color: "transparent" }}
										width="320"
									/>
								</div>
								<div className="space-y-0.5 pt-3">
									<p className="font-medium text-foreground">{member.name}</p>
									<p className="text-muted-foreground text-sm">{member.role}</p>
								</div>
							</div>
						))}
					</div>
				</div>
			</div>
		</section>
	);
}
