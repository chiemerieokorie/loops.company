import { defaultSocialMembers } from "./data";
import type { TeamMember } from "./types";

export interface TeamGridSocialProps {
	members?: TeamMember[];
	title?: string;
}
...
export function TeamGridSocial({
	title = "Our incredible leadership team",
	members = defaultSocialMembers,
}: TeamGridSocialProps) {
	return (
										...
										<div className="-mx-1 mt-auto flex gap-0.5">
											{member.socials.map((social) => (
												<a
													...
													className="flex size-7 items-center justify-center rounded text-muted-foreground hover:bg-foreground/5 hover:text-foreground"
													href={social.href}
													key={social.label}
													rel="noopener noreferrer"
													target="_blank"
												>
													{social.icon === "x" ? <XIcon /> : <LinkedInIcon />}
												</a>
											))}
										</div>
									)}
