...
export interface ExpandableFourCellTab {
	content: {
		title: string;
		description: string;
		ctaLabel?: string;
...
export interface ExpandableFourCellProps {
	autoplay?: boolean;
	badges?: string[];
	imageAlt?: string;
	imageUrl?: string;
	...
	tabs?: ExpandableFourCellTab[];
}

const CIRCUMFERENCE = 62.831_853_071_795_86;

export function ExpandableFourCell({
	tabs = [
		{
			label: "AI Models",
			duration: 6000,
		...
		<section className="@container overflow-hidden bg-background py-24">
			<div className="mx-auto my-1 max-w-5xl px-2 md:px-6">
				<div className="grid gap-px rounded-2xl bg-foreground/10 p-px md:grid-cols-2">
					{/* Top-left: feature description + tab nav */}
					<div className="relative rounded-[15px] bg-background p-6 sm:p-12">
					...
					<div className="relative h-fit">
						<div className="relative aspect-7/8 overflow-hidden rounded-[15px] bg-background">
							<div
								className="absolute inset-0 before:pointer-events-none before:absolute before:inset-0 before:z-10 before:rounded-[15px] before:border before:border-foreground/10"
								style={{ opacity: 1 }}
							>
					...
					{/* Bottom-left: badges */}
					<div className="rounded-[15px] bg-background p-6 sm:p-12">
					...
					{/* Bottom-right: quote */}
					<div className="flex flex-col justify-end rounded-[15px] bg-background p-6 sm:p-12">
