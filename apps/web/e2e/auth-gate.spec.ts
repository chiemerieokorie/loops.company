...
// component that calls `isAuthenticated()` and `redirect("/")` before rendering
// anything under the group. `/dashboard` lives there, so an unauthenticated visitor
// must never see the app shell — they land on the public landing page. This is the
// security boundary
...
test.describe("the (authed) route gate", () => {
	test("redirects an unauthenticated visitor from /dashboard to the landing page", async ({
		page,
	}) => {
